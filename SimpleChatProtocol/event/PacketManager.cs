...
public class PacketManager
{
    ...
    {
        if (!_packets.ContainsKey(packet.PacketId()))
        {
            return;
        }
        
        var normalPacket = _reader.ParsePacket(packet, Activator.CreateInstance(_packets[packet.PacketId()]) as IPacket);
        if (normalPacket == null)
        {
            return;
        }
        
        this.CallAndForget(normalPacket, eventArgs);
    }

    public async void CallAndForget(IPacket packet, EventArgs eventArgs)
    {
        if (!_handlers.ContainsKey(packet.PacketId()))
        {
            return;
        }
